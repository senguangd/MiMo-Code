...
import { AdpAuthPlugin } from "../../src/plugin/adp"
import type { PluginInput } from "@adp-ai/plugin"
...
describe("AdpAuthPlugin", () => {
  describe("config hook", () => {
    test("registers xiaomi provider placeholder", async () => {
      const hooks = await AdpAuthPlugin(fakeInput)
      const cfg: any = {}
  ...
  describe("auth hook structure", () => {
    test("registers auth for adp provider", async () => {
      const hooks = await AdpAuthPlugin(fakeInput)
  ...
  describe("authorize", () => {
    test("returns url with pk, redirect_uri, kn, key_name params", async () => {
      ...
      expect(url.searchParams.get("kn")).toBe("adpcli")
      expect(url.searchParams.get("key_name")).toMatch(/^adp-cli-cli-key-/)
  ...
  describe("chat.headers hook", () => {
    test("adds X-Adp-Source header for adp provider", async () => {
      const hooks = await AdpAuthPlugin(fakeInput)
      const output = { headers: {} as Record<string, string> }
      await hooks["chat.headers"]!({ model: { providerID: "xiaomi" } } as any, output as any)
      expect(output.headers["X-Adp-Source"]).toBe("adpcli-cli")
    })
    ...
    test("does not add header for other providers", async () => {
      ...
      for (const providerID of providers) {
        ...
        expect(output.headers["X-Adp-Source"]).toBeUndefined()
      }
    })
  })
