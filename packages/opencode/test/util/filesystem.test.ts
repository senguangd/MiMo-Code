...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "adpcli.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "adpcli.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "adpcli.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "adpcli.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["adpcli.json", "adpcli.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "adpcli.json"),
        path.join(tmp.path, "adpcli.jsonc"),
        path.join(project, "adpcli.json"),
        path.join(project, "adpcli.jsonc"),
      ])
    })
  })
