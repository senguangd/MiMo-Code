...
Body about cache.`
  ...
  itLive.live("union prune: deleting one CC file leaves adp rows untouched", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const memory = yield* Memory.Service
        const adpRoot = yield* memory.root()
        ...
        yield* Effect.promise(() => fs.writeFile(path.join(adpRoot, "global", "m.md"), "adp body"))
...
cc body`,
        ...
        expect(rows.length).toBe(1)
        expect(rows[0].scope).toBe("global") // adp row preserved
      }),
    ),
  )
...
body`,
        ...
        yield* Effect.promise(() => reconcileMemory({ adp: adpRoot }))
        expect(
          Database.use((db) =>
            db.select().from(MemoryFtsTable).where(eq(MemoryFtsTable.scope, "cc")).all(),
          ).length,
    ...
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.promise(() =>
          reconcileMemory({ adp: adpRoot, cc: "/definitely/not/a/real/path/abc123" }),
        )
