...
const opencodeClient = process.env.ADPCLI_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (opencodeClient === undefined) delete process.env.ADPCLI_CLIENT
  else process.env.ADPCLI_CLIENT = opencodeClient
})
