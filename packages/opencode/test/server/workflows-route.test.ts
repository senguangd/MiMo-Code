...
// process-global `:memory:` DB (test/preload sets ADPCLI_DB=:memory:, and the
...
// `x-adpcli-directory` header makes InstanceMiddleware re-enter the SAME cached
      ...
      Effect.fnUntraced(function* ({ dir }) {
        ...
        const response = yield* Effect.promise(async () =>
          Server.Default().app.request(`/workflows/wf_00000000000000000000000000/resume`, {
            method: "POST",
            headers: { "x-adpcli-directory": dir },
          }),
        )
