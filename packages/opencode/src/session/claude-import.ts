...
export async function run(opts?: { force?: boolean }) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      // user deleted it in adpcli, drop the stale mapping and import fresh —
      ...
      Database.transaction((tx) => {
        ...
        if (existing) {
          ...
          // user's adpcli-native continuation messages in the same session.
          ...
          // Preserve adpcli-owned metadata on re-sync: keep any user rename
