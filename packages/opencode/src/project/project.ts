...
async function setupProjectIdEnvironment(workingDir: string): Promise<void> {
  ...
  const localFile = nodePath.join(workingDir, ".adpcli-project-id")
  const idFile = nodePath.join(mainGit, "adpcli-project-id")
  ...
  // Belt-and-suspenders: ensure .git/info/exclude lists .adpcli-project-id
  ...
  if (!existing.includes(".adpcli-project-id")) {
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.ADPCLI_FAKE_VCS)
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const data: DiscoveryResult = yield* Effect.gen(function* () {
        ...
        // turn, which has shipped as a bug for users opening adpcli in $HOME.
      ...
      if (Flag.ADPCLI_EXPERIMENTAL_ICON_DISCOVERY) yield* discover(existing).pipe(Effect.ignore, Effect.forkIn(scope))
