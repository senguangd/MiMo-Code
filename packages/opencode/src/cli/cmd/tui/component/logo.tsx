...
export function Logo(props: { shape?: LogoShape; ink?: RGBA; idle?: boolean; sweep?: boolean } = {}) {
  ...
  const ADP_ORANGE = RGBA.fromInts(251, 129, 71)
  const ADP_GRAY = RGBA.fromInts(160, 160, 160)
          ...
          if (labelRow) {
            return (
              <box flexDirection="row" gap={1}>
                <text fg={ADP_GRAY} selectable={false}>{line}</text>
                <text fg={ADP_GRAY} selectable={false}>{ctx.shape.right[index()]}</text>
              </box>
            )
          }
          return (
            <box flexDirection="row" gap={1}>
              <box flexDirection="row">
                {renderLine(line, index(), props.ink ?? ADP_ORANGE, true, 0, frame(), dusk(), idleState())}
              </box>
              <box flexDirection="row">
                {renderLine(
