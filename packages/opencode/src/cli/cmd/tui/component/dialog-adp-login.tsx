...
export function DialogAdpLogin() {
    ...
    const recommended = [
      {
        ...
        onSelect: async () => {
          ...
          dialog.replace(() => (
            <AdpOAuthFlow url={result.data!.url} instructions={result.data!.instructions} />
          ))
        },
      },
      {
...
function AdpOAuthFlow(props: { url: string; instructions: string }) {
  const dialog = useDialog()
  const sdk = useSDK()
  const sync = useSync()
  const local = useLocal()
  ...
  async function onLoginSuccess() {
    ...
    const defaultModel = xiaomi && "adp-v2.5-pro" in xiaomi.models ? "adp-v2.5-pro" : xiaomi ? Object.keys(xiaomi.models)[0] : undefined
    if (defaultModel) {
