...
export const dict = {
  ...
  "tui.home.agreement.prefix": "AdpCli をご利用いただくことで、",
  ...
  "tui.tips.config_files":
    "サーバ設定は {highlight}adpcli.json{/highlight}、TUI 設定は {highlight}tui.json{/highlight} を作成します",
  "tui.tips.global_config":
    "TUI 設定をグローバルに置くなら {highlight}~/.config/adpcli/tui.json{/highlight} を使います",
  ...
  "tui.tips.mcp_oauth": "AdpCli は認証が必要なリモート MCP サーバの OAuth を自動処理します",
  "tui.tips.custom_command":
    "{highlight}.adpcli/command/{/highlight} に {highlight}.md{/highlight} ファイルを追加すると再利用可能なカスタムプロンプトを定義できます",
  ...
  "tui.tips.custom_agent":
    "{highlight}.adpcli/agent/{/highlight} に {highlight}.md{/highlight} ファイルを追加して専用 AI ペルソナを作れます",
  ...
  "tui.tips.formatter": "AdpCli は prettier、gofmt、ruff などでファイルを自動整形します",
  ...
  "tui.tips.lsp": "AdpCli はインテリジェントなコード解析に LSP サーバを使います",
  "tui.tips.custom_tool":
    "{highlight}.adpcli/tools/{/highlight} に {highlight}.ts{/highlight} ファイルを作って新しい LLM ツールを定義します",
  "tui.tips.tool_scripts": "ツール定義から Python、Go などのスクリプトを呼び出せます",
  "tui.tips.plugins":
    "{highlight}.adpcli/plugin/{/highlight} に {highlight}.ts{/highlight} ファイルを追加してイベントフックを実装できます",
  "tui.tips.plugin_notify": "プラグインでセッション完了時に OS 通知を送れます",
  "tui.tips.plugin_protect": "プラグインを書いて AdpCli が機密ファイルを読まないようにできます",
  "tui.tips.run": "{highlight}adp run{/highlight} で非対話的なスクリプト実行ができます",
  "tui.tips.continue": "{highlight}adp --continue{/highlight} で前回のセッションを再開します",
  "tui.tips.attach_cli": "{highlight}adp run -f file.ts{/highlight} で CLI からファイルを添付できます",
  "tui.tips.format_json": "{highlight}--format json{/highlight} を使うとスクリプトで機械可読な出力を得られます",
  "tui.tips.serve": "{highlight}adp serve{/highlight} を実行するとヘッドレスで AdpCli API にアクセスできます",
  "tui.tips.attach_server": "{highlight}adp run --attach{/highlight} で起動中のサーバに接続します",
  "tui.tips.upgrade": "{highlight}adp upgrade{/highlight} で最新バージョンに更新します",
  "tui.tips.auth_list": "{highlight}adp auth list{/highlight} で設定済みプロバイダを確認します",
  "tui.tips.agent_create": "{highlight}adp agent create{/highlight} でガイド付きエージェント作成を行います",
  "tui.tips.github_install": "{highlight}adp github install{/highlight} で GitHub workflow を設定します",
  ...
  "tui.tips.theme_files": "{highlight}.adpcli/themes/{/highlight} ディレクトリに JSON テーマファイルを作成します",
  ...
  "tui.tips.debug_config": "{highlight}adp debug config{/highlight} で設定の問題を調査します",
  ...
  "tui.tips.zen": "Adp Cli と組み合わせて {highlight}/connect{/highlight} で厳選されたモデルを使えます",
  ...
  "tui.skill.adpcli.description": "AdpCli の機能・設定・コマンドに関するセルフドキュメント",
  ...
  "tui.skill.drive-adp.description": "別の AdpCli プロセスをプログラムで操作——ヘッドレス JSON イベントまたは tmux 経由のインタラクティブ TUI",
  ...
  "tui.model.adp_auto.name": "Adp Auto（Adp-V2.5 期間限定無料）",
  ...
  "tui.dialog.try_best.handoff.description": "未完了の作業をこのハーネスへ委任するよう Adp に依頼",
  ...
  "tui.toast.update_available.success": "AdpCli v{{version}} に更新しました。アプリケーションを再起動してください。",
  ...
  "tui.voice.error.no_auth": "/connect で Adp アカウントに接続するか、voice.asr_model で別のプロバイダーを設定してください",
  ...
  "tui.dialog.login.adp_free.desc": "ログイン不要の匿名無料チャネル",
  "tui.dialog.login.adp_free.success": "Adp Auto (free) の準備完了 — デフォルトモデルを adp/adp-auto に設定しました",
  "tui.dialog.login.adp_free.unavailable": "Adp Auto (free) プロバイダーが読み込まれていません",
  "tui.dialog.login.flow.title": "Adp ログイン",
  ...
  "cli.providers.adp.recommended_hint": "推奨",
  "cli.providers.adp_free.hint": "匿名無料チャネル / adp-auto",
  "cli.providers.adp_free.verifying": "Adp Auto (free) チャネルを検証中...",
  "cli.providers.adp_free.ready": "Adp Auto (free) チャネル準備完了",
  "cli.providers.adp_free.failed": "Adp Auto (free) の自己診断に失敗",
  "cli.providers.adp_free.default_set": "デフォルトモデルを adp/adp-auto に設定（1M コンテキスト、無料）",
  "cli.providers.adp_free.usage_hint":
    "ログイン不要 — そのまま adp を実行できます。有料/上位モデルを利用する場合は Adp ブラウザログインを選択してください。",
  "cli.providers.adp_login.decrypt_retry": "復号に失敗しました、再試行してください（残り {remaining} 回）",
  "cli.providers.adp_login.decrypt_exhausted": "復号に失敗しました、最大再試行回数に達しました",
  ...
  "trust.capabilities": "Adp Cli はここでファイルの読み取り、編集、実行が可能になります。",
  ...
  "skip_permissions.body":
    "--dangerously-skip-permissions を指定して起動しました。Adp Cli は承認を求めずにファイルの読み取り・編集・実行やシェルコマンドの実行を行います。設定で明示的に拒否（deny）したルールのみが引き続き適用されます。発生したいかなる結果についてもあなた自身が全責任を負います。",
