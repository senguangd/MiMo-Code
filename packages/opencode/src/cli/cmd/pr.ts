...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run adpcli",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for adpcli session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found adpcli session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["adp", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting adpcli...")
        UI.println()

        const adpArgs = sessionId ? ["-s", sessionId] : []
        const adpProcess = Process.spawn(["adp", ...adpArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await adpProcess.exited
        if (code !== 0) throw new Error(`adp exited with code ${code}`)
      },
    })
  },
})
