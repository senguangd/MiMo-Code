---
name: drive-adp
description: Use when you need to programmatically drive another AdpCli (adp) process — supports both headless `adp run` with JSON events and interactive TUI via tmux for full terminal interaction testing. Covers driving either an installed `adp` binary or a dev build launched from source with `bun dev` (for debugging adpcli itself). Reach for it to script, test, or automate a separate adp instance and validate its behavior from parseable evidence.
---
...
Two **interfaces** for driving a separate adp process:

| Interface | Command | Use when |
|------|---------|----------|
| **Headless** | `adp run --format json` | Scripted tasks, CI, event validation |
| **TUI** | `adp` in tmux | Interactive flow, permission dialogs, keybindings, visual regression |

**Core principle:** Every run produces parseable evidence. No eyeballing.

Always drive an *isolated* instance: give each run a fresh `ADPCLI_HOME` and a throwaway workspace so it never touches your own config, memory, or session DB.
...
|----------|---------|----------|
| **Installed binary** | `adp …` | Testing a released/installed build |
| **Dev (from source)** | `bun dev …` | Debugging adpcli itself — runs `src/index.ts` directly, no build step, picks up local code changes |

Everything below uses `adp` for brevity. To drive a **dev build** instead,
substitute `bun dev` for `adp` and run from the repo root — every flag,
JSON event, and tmux technique is identical. See the Dev Mode section.

## Prerequisites

```bash
# adp binary must be on PATH (installed-binary launcher)
which adp || echo "adp not found on PATH"

# OR: dev launcher — run from the adpcli repo root, needs bun
...
## Dev Mode (debugging adpcli itself)

When the goal is to debug **adpcli's own code**, launch from source with
`bun dev` instead of the installed `adp` binary. It runs
...
- It is the dev equivalent of the `adp` command: same CLI, same subcommands
...
- Args pass straight through, so **both interfaces work under dev**:
  ...
  - TUI:      `bun dev <workspace>` (positional workspace arg, as with `adp`)
- If `ADPCLI_HOME` is not set, dev defaults it to a repo-local `.dev-home`
  dir. For an isolated driven run, set `ADPCLI_HOME=$(mktemp -d)` explicitly
  just like with the binary.

**Substitution rule:** anywhere Part 1 / Part 2 / Part 3 below say `adp`,
replace it with `bun dev` (invoked from the repo root) to drive a dev build.

```bash
# Headless, dev build (from repo root)
REPO=/path/to/adpcli/checkout   # your local adpcli repo root
ADPCLI_HOME=$(mktemp -d) bun --cwd "$REPO" dev run \
  --format json --dangerously-skip-permissions --dir "$WORKSPACE" \
  < "$PROMPT" > /tmp/adp-dev.jsonl 2>&1

# TUI, dev build in tmux (from repo root)
tmux new-session -d -s "$SESSION" -x 120 -y 30 \
  "cd $REPO && ADPCLI_HOME=$(mktemp -d) ADPCLI_PURE=true bun dev $WORKSPACE; sleep 999"
```

---

## Part 1: Headless Mode (`adp run`)

### Launch

```bash
PROMPT=$(mktemp -t adp-drive.XXXXXX)
cat >"$PROMPT" <<'EOF'
Your task here.
EOF

ADPCLI_HOME=$(mktemp -d) adp run \
  --format json \
  --dangerously-skip-permissions \
  --dir "$WORKSPACE" \
  < "$PROMPT" > /tmp/adp-out.jsonl 2>&1
...
# No errors
grep -q '"type":"error"' /tmp/adp-out.jsonl && echo "FAIL: errors found"

# A specific tool was used (match part.tool, the bare string)
grep -q '"tool":"write"' /tmp/adp-out.jsonl || echo "FAIL: write tool not called"

# Text output contains expected string (text is at .part.text)
grep '"type":"text"' /tmp/adp-out.jsonl | jq -r '.part.text' | grep -q "expected"

# Robust tool check via jq (works regardless of key ordering)
jq -e 'select(.type=="tool_use") | .part.tool=="write"' /tmp/adp-out.jsonl >/dev/null \
...
```bash
timeout 120 adp run --format json --dangerously-skip-permissions < "$PROMPT"
...
| Variable | Purpose |
|---|---|
| `ADPCLI_HOME` | **Required.** Fresh `mktemp -d` per run. Sandboxes DB, config, cache. |
| `ADPCLI_PURE=true` | Disable external plugins. |
| `ADPCLI_DISABLE_GIT=true` | Skip git ops if workspace isn't a real repo. |
...
# Launch adp TUI in tmux (workspace is a positional arg, NOT --dir)
tmux new-session -d -s "$SESSION" -x 120 -y 30 \
  "ADPCLI_HOME=$MHOME ADPCLI_PURE=true adp $WORKSPACE; sleep 999"
...
test_smoke() {
  ...
  echo "Say hello" > "$P"
  ADPCLI_HOME=$MHOME adp run --format json --dangerously-skip-permissions \
    < "$P" > /tmp/s1.jsonl 2>&1
  local E=$?
  rm -rf "$MHOME" "$P"
...
test_tool_use() {
  ...
  ADPCLI_HOME=$MHOME adp run --format json --dangerously-skip-permissions --dir "$WS" \
    < "$P" > /tmp/s2.jsonl 2>&1
  local E=$?
  local OK=true
  [ $E -ne 0 ] && OK=false
...
test_tui_interactive() {
  local MHOME=$(mktemp -d) WS=$(mktemp -d) SID="adp-s3-$$"
  local PROMPT_RE='>|Ask|/[a-z]'
  tmux new-session -d -s "$SID" -x 120 -y 30 "ADPCLI_HOME=$MHOME ADPCLI_PURE=true adp $WS; sleep 999"
...
```bash
test_tui_permission() {
  local MHOME=$(mktemp -d) WS=$(mktemp -d) SID="adp-s4-$$"
  local PROMPT_RE='>|Ask|/[a-z]'
  tmux new-session -d -s "$SID" -x 120 -y 30 "ADPCLI_HOME=$MHOME ADPCLI_PURE=true adp $WS; sleep 999"
...
test_tui_keybindings() {
  local MHOME=$(mktemp -d) WS=$(mktemp -d) SID="adp-s5-$$"
  local PROMPT_RE='>|Ask|/[a-z]'
  tmux new-session -d -s "$SID" -x 120 -y 30 "ADPCLI_HOME=$MHOME ADPCLI_PURE=true adp $WS; sleep 999"
...
| Task | Command |
|------|---------|
| Headless run | `ADPCLI_HOME=$(mktemp -d) adp run --format json --dangerously-skip-permissions "prompt"` |
| TUI launch | `tmux new-session -d -s test -x 120 -y 30 "ADPCLI_HOME=$(mktemp -d) adp $WORKSPACE; sleep 999"` |
