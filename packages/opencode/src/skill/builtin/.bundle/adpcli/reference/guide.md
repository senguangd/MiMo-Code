...
1. **Sign in** — `adp account login <url>` runs a device flow: it prints a URL + code and opens your browser. `/connect` does the same from inside the TUI (e.g. to add OpenRouter). Other account subcommands: `logout`, `switch`, `orgs`, `open`, `console`.
2. **Pick a model** — set `"model": "provider/model"` in config, or switch live in the TUI model dialog. Provider API keys are auto-detected from environment variables (unless `ADPCLI_ADP_ONLY=1`).
3. **List what's available** — `adp models`, `adp providers`.

## Memory: making AdpCli remember
...
Drop a markdown file at `.adpcli/command/<name>.md` (or `.adpcli/commands/`, `.claude/command(s)/` are also read). The frontmatter configures it; the body is the prompt template.
...
A "mode" is just a **primary agent** with its own system prompt. To give AdpCli a custom mode (e.g. a `general` chat mode alongside the coding-focused `build`), drop a markdown file — no code, no server changes. The frontmatter is config; the **markdown body becomes the agent's system prompt**.
...
| Path | Scope |
|------|-------|
| `.adpcli/agent/<name>.md` (or `agents/`) | project agent — most common |
| `.adpcli/mode/<name>.md` (or `modes/`) | project mode — same as an agent forced to `mode: primary` |
| `~/.config/adpcli/agent/<name>.md` | global agent, available in every project |
...
Verify a file loaded with `adp agent list` — your agent shows up with its `(primary)` / `(subagent)` mode.
...
Inspect/manage with `adp mcp`. Request timeout defaults to 5000ms (`timeout` per server, or `experimental.mcp_timeout` globally).
...
**Cron** schedules a prompt to be injected into a session on a recurring or one-shot basis. It is driven by the `cron` tool (no `/cron` slash command); `ADPCLI_EXPERIMENTAL_CRON` is **on by default** (kill switch: `ADPCLI_DISABLE_CRON`).
...
- **Durable** jobs persist to `<project>/.adpcli/scheduled_tasks.json` and survive restart; non-durable jobs live only for the session.
...
To add tools, hooks, or skills, use the `evolve` skill — it covers writing `.adpcli/tools/*.ts`, `.adpcli/hooks/*.ts`, and `.adpcli/skills/*/SKILL.md`, all hot-reloaded on the next turn.
