...
| Surface | How |
|---------|-----|
| TUI (`adp`) | `adp --dangerously-skip-permissions` |
| Headless (`adp run`) | `adp run --dangerously-skip-permissions "<prompt>"` |
| Any surface (env) | `ADPCLI_PERMISSION='"allow"'` or `ADPCLI_DANGEROUSLY_SKIP_PERMISSIONS=1` |
...
- A top-level catch-all like `"permission": { "*": "ask" }` makes the TUI/env form a no-op (your `*: ask` outranks the injected `*: allow`). Remove it, or use `adp run --dangerously-skip-permissions`, which auto-replies at the event layer and overrides `ask` too.
...
- `external_directory` governs reads/writes outside the project working directory — by default these prompt, so AdpCli never silently widens scope.
