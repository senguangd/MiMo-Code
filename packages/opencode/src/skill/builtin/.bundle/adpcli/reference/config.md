# AdpCli Configuration Reference

## File locations & precedence

Config is JSON or JSONC. AdpCli discovers it by walking up from the cwd to the worktree root, then falls back to global.

- Project: `.adpcli/adpcli.json` or `.adpcli/adpcli.jsonc`
- Global: `~/.config/adpcli/adpcli.json` (XDG config dir)
- Extra config dirs are also searched via `$ADPCLI_CONFIG_DIR`.

Project config merges **over** global. Always include `"$schema": "https://adp.xiaomi.com/adpcli/config.json"` for validation.

## On-disk data layout

Base directories resolve from `ADPCLI_HOME` if set (must be absolute → `<home>/{data,cache,config,state}`), otherwise from XDG:

| Kind | Default location | Holds |
|------|------------------|-------|
| data | `~/.local/share/adpcli/` | memory, logs, `builtin_skills/<version>/`, bin |
| config | `~/.config/adpcli/` | global `adpcli.json` |
| cache | `~/.cache/adpcli/` | caches, downloaded bins |
| state | `~/.local/state/adpcli/` | runtime state |

Memory files live under `~/.local/share/adpcli/memory/`:
...
- `ADPCLI_HOME` — override all base dirs (absolute path).
- `ADPCLI_CONFIG_DIR` — extra config directory to search.
- `ADPCLI_PURE` — run without external plugins (same as `adp --pure`). Does **not** change models or Claude Code inheritance.
- `ADPCLI_ADP_ONLY` — pure-Adp mode: don't inherit Claude Code settings (CLAUDE.md, `~/.claude/skills`), don't read provider API keys from env, fall back to the adp-auto model.
- `ADPCLI_DISABLE_LOG_ROTATION` — keep a single growing log file instead of rotating.
- `ADPCLI_TEXT_TOOL_CALL_RETRY_LIMIT` — retries when a model emits a tool call as prose markup instead of a structured call (default 2).
- `ADPCLI_EXPERIMENTAL_CRON` — scheduled prompts (cron/loop); **on by default**. `ADPCLI_DISABLE_CRON` kills it at runtime. Tune loop keepalive with `ADPCLI_LOOP_KEEPALIVE_BUDGET` (default 1) and `ADPCLI_LOOP_KEEPALIVE_DELAY_S` (default 1200).
- `ADPCLI_EXPERIMENTAL_TOKEN_EFFICIENCY_HEURISTIC` — shape-based compaction of bash output to save tokens; off by default.
- `ADPCLI_DISABLE_BUILTIN_SKILLS`, `_COMPOSE_SKILLS`, `_EXTERNAL_SKILLS`, `_CLAUDE_CODE_SKILLS`, `_CODEX_SKILLS`, `_OPENCODE_SKILLS`, `_PROJECT_CONFIG`, `_CLAUDE_IMPORT` — feature toggles.
...
```jsonc
{
  ...
  "model_groups": {
    ...
    "standard": {
      ...
      "models": ["anthropic/claude-sonnet", "openrouter/xiaomi/adp-v2.5"]
    },
...
- A ref without `/` is a group name. If configured, AdpCli is **provider-aware**: it prefers a member on the caller's current provider, otherwise falls back to the group's `default`.
...
Prefer a markdown file (`.adpcli/agent/<name>.md`, body = system prompt) for defining a custom agent/mode — see the "Custom agents & modes" section in @guide.md. Use the `agent` config key for short, inline per-agent overrides.
...
| `voice.asr_model` | ASR model (default `xiaomi/adp-v2.5-asr`) |
| `voice.control_model` | Voice control model (default `xiaomi/adp-v2.5`) |
