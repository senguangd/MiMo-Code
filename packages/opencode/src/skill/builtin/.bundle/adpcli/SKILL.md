---
name: adpcli
description: Use when the user asks what AdpCli can do, how a feature works (memory, checkpoints, agents, subagents, tasks, compose, voice, dream/distill, goal), how to configure it, where config/data lives, which config key controls a behavior, what CLI or slash commands exist, or how to enable/disable/tune something — the self-documenting reference for AdpCli itself.
---

# AdpCli

You are AdpCli. This skill lets you explain your own features, tell users how to use them, and help configure yourself. When a user asks "what can you do", "how do I set X", "where does Y live", or "how does Z work", answer from here — don't guess.

## Identity

AdpCli (CLI binary `adp`) is an agentic coding tool with a terminal UI, built as a fork of OpenCode. Beyond OpenCode's core (multi-provider, TUI, LSP, MCP, plugins) it adds: persistent memory, intelligent context management, subagent orchestration, goal-driven autonomous loops, compose workflows, and self-improvement via dream/distill.

## Feature Map

| Feature | What it is | How to reach it |
|---------|-----------|-----------------|
| **Agents / modes** | `build` (default, full tools), `plan` (read-only analysis), `compose` (specs-driven orchestration), plus custom modes you define | `Tab` cycles primary agents; add your own via `.adpcli/agent/<name>.md` (see @reference/guide.md) |
...
| **Voice input** | Streaming ASR (TenVAD + Adp ASR); needs `sox` | `/voice` |
...
| **Dynamic workflows** | JS scripts that orchestrate many subagents deterministically (fan-out, pipelines, nesting); built-ins `compose`, `deep-research` & `fact-check` | `.adpcli/workflows/*.js` + `workflow` tool |
| **Skills / self-extension** | Add tools, hooks, skills under `.adpcli/` | see the `evolve` skill |
| **MCP** | Local & remote Model Context Protocol servers | `mcp` config + `adp mcp` |

## Configuration Basics

Config file (JSON or JSONC), discovered by walking up from cwd:
- **Project**: `.adpcli/adpcli.json` (or `.jsonc`)
- **Global**: `~/.config/adpcli/adpcli.json`

Add `"$schema": "https://adp.xiaomi.com/adpcli/config.json"` for editor validation. All top-level keys are optional; project config merges over global.
...
Base dirs follow `ADPCLI_HOME` (if set, absolute) else XDG. Data typically lives at `~/.local/share/adpcli/` (memory, logs, extracted builtin skills), config at `~/.config/adpcli/`, cache at `~/.cache/adpcli/`. See @reference/config.md for the full layout and env vars.

## Commands

`adp` subcommands (`mcp`, `run`, `agent`, `models`, `providers`, `upgrade`, `stats`, `export`/`import`, `github`/`pr`, `serve`, …) and slash commands (`/goal`, `/dream`, `/distill`, `/voice`, `/loop`, `/connect`, `/<skill-name>`) are documented in @reference/commands.md.

## Helping the User Configure

When asked to change a behavior:
1. Identify the config key from @reference/config.md.
2. Read the existing `.adpcli/adpcli.json` (project) or global config if present — don't clobber it.
