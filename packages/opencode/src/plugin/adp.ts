import type { Hooks, PluginInput } from "@adp-ai/plugin"
...
const log = Log.create({ service: "plugin.adp" })

const PLATFORM_URL = process.env.ADP_PLATFORM_URL || "https://platform.xiaomiadp.com"

function getKeyName(): string {
  const filePath = path.join(Global.Path.data, "adp-key-name")
  try {
    const existing = fs.readFileSync(filePath, "utf-8").trim()
    if (existing) return existing
  } catch {}
  const name = `adp-cli-cli-key-${crypto.randomBytes(4).toString("hex")}`
...
export async function AdpAuthPlugin(_input: PluginInput): Promise<Hooks> {
  return {
    config: async (input) => {
      input.provider ??= {}
      ...
      // api: https://api.xiaomiadp.com/v1) — hardcoding "Adp" here collided with
      // the free "adp" provider's display name and confused users.
      ...
      methods: [
        {
          ...
          authorize: async () => {
            ...
            log.info("adp oauth server started", { port })
            ...
            const serverCallbackPromise = new Promise<{ sk?: string; uid: string; url?: string }>((resolve, reject) => {
              ...
              server.on("request", (req, res) => {
                ...
                log.info("adp oauth callback received", { path: url.pathname, query: url.search.substring(0, 100) })

                const u = url.searchParams.get("u")

                if (!u) {
                  log.warn("adp oauth callback missing u param")
                ...
                try {
                  ...
                  log.info("adp oauth decrypt success", { uid: result.uid, url: result.url })
                  res.writeHead(302, { Location: `${PLATFORM_URL}/authorize/callback?status=success` })
                  res.end()
                  clearTimeout(timeout)
                  resolve(result)
                } catch (err) {
                  log.error("adp oauth decrypt failed", { error: err })
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers["X-Adp-Source"] = "adpcli-cli"
    },
  }
}
