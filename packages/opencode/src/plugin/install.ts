...
export type PatchDeps = {
  ...
  files: (dir: string, name: "adpcli" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".adpcli")
}

function patchName(kind: Kind): "adpcli" | "tui" {
  if (kind === "server") return "adpcli"
  return "tui"
}
...
async function patchOne(dir: string, target: Target, spec: string, force: boolean, dep: PatchDeps): Promise<PatchOne> {
  ...
  await using _ = await Flock.acquire(ConfigPaths.lockKey(dir, name))
