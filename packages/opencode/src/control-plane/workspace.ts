...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    ADPCLI_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    ADPCLI_WORKSPACE_ID: config.id,
    ADPCLI_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.ADPCLI_EXPERIMENTAL_WORKSPACES) return
