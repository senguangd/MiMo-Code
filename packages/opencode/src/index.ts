...
const cli = yargs(args)
  ...
  .middleware(async (opts) => {
    ...
    process.env.AGENT = "1"
    process.env.ADPCLI = "1"
    process.env.ADPCLI_PID = String(process.pid)

    Log.Default.info("adpcli", {
    ...
    const marker = path.join(Global.Path.data, "adpcli.db")
    ...
    if (!process.env.ADPCLI_DISABLE_CLAUDE_IMPORT && !process.env.ADPCLI_CLAUDE_IMPORTED) {
      process.env.ADPCLI_CLAUDE_IMPORTED = "1"
      try {
    ...
    if (process.platform === "win32") {
      try {
        ...
        for (const entry of readdirSync(binDir)) {
          if (entry.startsWith("adp.exe.old_")) {
