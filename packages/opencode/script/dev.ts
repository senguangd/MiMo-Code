...
// checkout (at ../../adpapi/packages/opencode/src/ext), it is copied into
...
const overlaySrc = path.resolve(pkgDir, "../../adpapi/packages/opencode/src/ext")
...
const proc = Bun.spawn(["bun", "run", "--conditions=browser", "src/index.ts", ...process.argv.slice(2)], {
  ...
  env: { ...process.env, ADPCLI_HOME: process.env.ADPCLI_HOME ?? path.resolve(pkgDir, "../../.dev-home") },
})
