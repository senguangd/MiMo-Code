...
| `toolCapabilities` | Object |          | Explicit semantic capabilities keyed by the MCP server's original tool name.        |
...
Declare semantic capabilities only when an MCP tool should be advertised to workflows as web search, web fetch, or code/documentation search. Use the original tool name returned by that MCP server:
...
```jsonc title="opencode.jsonc"
{
  "mcp": {
    "one-search": {
      "type": "local",
      "command": ["one-search-mcp"],
      "toolCapabilities": {
        "one_search": ["web-search"],
        "one_scrape": ["web-fetch"]
      }
    }
  }
}
...
Supported values are `web-search`, `web-fetch`, and `code-search`. MiMo Code never infers a capability from a tool name and does not rename or alias the MCP tool; the model must call the exact generated tool ID and follow its schema.
