...
Supported values are `web-search`, `web-fetch`, and `code-search`. Adp Cli never infers a capability from a tool name and does not rename or alias the MCP tool; the model must call the exact generated tool ID and follow its schema.
