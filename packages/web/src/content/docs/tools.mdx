...
:::note
This tool is available when Xiaomi native search credentials are configured, when using the OpenCode provider, or when the `MIMOCODE_ENABLE_EXA` environment variable is set to a truthy value (for example, `true` or `1`). The legacy `OPENCODE_ENABLE_EXA` name remains accepted as a compatibility alias.

To enable Exa explicitly when launching MiMo Code:

```bash
MIMOCODE_ENABLE_EXA=1 mimo
```
