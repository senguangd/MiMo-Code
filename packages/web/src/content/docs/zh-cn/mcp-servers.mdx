...
| `toolCapabilities` | 对象 |      | 按 MCP 服务器返回的原始工具名显式声明语义能力。                   |
...
只有当某个 MCP 工具确实应被工作流识别为网页搜索、网页获取或代码/文档搜索时，才显式声明其语义能力。键必须使用该 MCP 服务器返回的原始工具名：
...
支持的值为 `web-search`、`web-fetch` 和 `code-search`。MiMo Code 不会根据工具名称猜测能力，也不会重命名或为 MCP 工具创建别名；模型必须调用最终生成的精确工具 ID，并遵循其 schema。
