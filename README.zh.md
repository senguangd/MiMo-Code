...
<p align="center">
  <a href="https://adp.xiaomi.com/zh/adpcli">官网</a> | <a href="https://adp.xiaomi.com/zh/blog/adp-cli-long-horizon">博客</a>
</p>

---

AdpCli 是一个终端原生的 AI 编程助手。它能读写代码、执行命令、管理 Git，通过持久化记忆系统，在多次会话间保持对你项目的深度理解，并自我进化。

内置 Adp Auto 限时免费通道——零配置即可开始使用。也支持接入各家主流 LLM 厂商 API。
...
中日韩字符可能显示为乱码。AdpCli 已为 PowerShell/cmd 子进程强制开启 UTF-8 输出。
...
除了 AdpCli，小米 Adp 模型也能在 Cursor、Cline、Zed 等各种 Agent 和编程工具里使用。

**[awesome-adp-agent](https://github.com/XiaomiAdp/awesome-adp-agent)** 收集了这些工具接入 Adp 模型的配置教程，想换个工具试试 Adp 的话可以去看看。也欢迎把你自己的接入方式提 PR 分享出来。
...
**自定义 Workflow：** 在 `.adpcli/workflows/` 或 `.claude/workflows/` 下放置 `.js` 文件即可定义自己的 Workflow，也可用同名文件覆盖内置 Workflow（如 `.adpcli/workflows/compose.js`）。

### 内置技能（Builtin Skills）

技能（Skill）是可复用的指令集，教会 Agent 如何处理特定任务（如生成 PDF、写学术论文、搜索 arXiv）。AdpCli 内置以下技能：
...
| `loop` | 按固定周期调度循环提示 |
| `adpcli` | AdpCli 功能和配置的自文档参考 |

**覆盖内置技能：** 在项目（`.adpcli/skills/<name>/SKILL.md`）或个人技能目录（`~/.claude/skills/`、`~/.opencode/skills/` 等）中创建同名技能即可。扫描顺序中后发现的用户技能会覆盖同名的内置技能。
...
| 变量 | 效果 |
|------|------|
| `ADPCLI_DISABLE_BUILTIN_SKILLS=true` | 禁用所有内置技能 |
| `ADPCLI_DISABLE_OFFICIAL_SKILLS=true` | 仅禁用办公/媒体类技能：`docx-official`、`pdf-official`、`pptx-official`、`xlsx-official`、`html-to-video-pipeline` |
...
基于 TenVAD 和 Adp ASR 的实时流式语音输入。通过 `/voice` 激活，按停顿分片转写，文本逐段追加到输入框。仅对 Adp 登录用户可用。需要安装 `sox`（macOS 上 `brew install sox`，其他平台类似）。
...
<details>
<summary><strong>非 Adp 渠道语音输入（OpenRouter、内部 API 等）</strong></summary>

语音输入可通过 `voice` 配置字段路由到其他 OpenAI 兼容 provider。ASR 模型（`adp-v2.5-asr`）仅在 Adp 平台可用；语音控制模式（`adp-v2.5`）可通过 OpenRouter 等中转平台使用。
...
> **注意**：自定义 provider 注册的模型会出现在主模型选择列表中。请勿将 ASR 专用模型（如 `adp-v2.5-asr`）用作编程主模型。
...
AdpCli 使用 JSON/JSONC 配置文件，并提供 JSON Schema 以获得编辑器自动补全和校验。

### 文件位置

| 文件 | 项目级 | 全局 |
|------|--------|------|
| 主配置 | `.adpcli/adpcli.jsonc` | `~/.config/adpcli/adpcli.json` |
| TUI 配置 | `.adpcli/tui.json` | `~/.config/adpcli/tui.json` |
| 认证凭据 | — | `~/.local/share/adpcli/auth.json` |

> Windows 下 XDG 路径位于 `%LOCALAPPDATA%\adpcli\`。可通过 `ADPCLI_HOME` 环境变量覆盖所有路径。

### JSON Schema

AdpCli 在首次加载配置时会自动注入 `$schema` 字段，使编辑器开箱即获得补全和校验：
...
除配置文件外，AdpCli 在 XDG 路径（或 `$ADPCLI_HOME`）下存储运行时数据：

| 目录 | 默认路径（Linux） | 内容 |
|------|------------------|------|
| data | `~/.local/share/adpcli/` | SQLite 数据库、认证凭据（`auth.json`）、记忆、日志 |
| state | `~/.local/state/adpcli/` | TUI 偏好设置（`kv.json`）、最近使用模型（`model.json`） |
| cache | `~/.cache/adpcli/` | 语言服务器、缓存的模型目录、技能 |

如需删除已存储的凭据，删除 data 目录下的 `auth.json` 即可。macOS 下 XDG data 默认为 `~/Library/Application Support/adpcli/`。
...
这是有意为之：AdpCli 不会静默放宽权限，你始终掌控模型在项目之外能触碰什么。
...
# 无头模式
adp run --dangerously-skip-permissions "你的提示词"
...
AdpCli 基于 [OpenCode](https://github.com/anomalyco/opencode) fork 构建，保留其全部核心能力（多 Provider、TUI、LSP、MCP、插件），并在此基础上构建了持久化记忆、智能上下文管理、子智能体编排、目标驱动的自主循环、Compose 工作流，以及通过 dream/distill 实现的自我进化。
...
使用 AdpCli 还需遵守[使用限制](./USE_RESTRICTIONS.md)。
使用小米 Adp 托管服务须遵守 [Adp 服务条款](https://platform.xiaomiadp.com/docs/terms/user-agreement)。
使用 Adp 名称、标志和商标须遵守 Adp 商标政策。
